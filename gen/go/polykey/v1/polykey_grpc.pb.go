...
const (
	PolykeyService_ExecuteTool_FullMethodName       = "/polykey.v1.PolykeyService/ExecuteTool"
	PolykeyService_ExecuteToolStream_FullMethodName = "/polykey.v1.PolykeyService/ExecuteToolStream"
)
...
type PolykeyServiceClient interface {
	...
	ExecuteToolStream(ctx context.Context, in *ExecuteToolStreamRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[ExecuteToolStreamResponse], error)
}
...
func (c *polykeyServiceClient) ExecuteToolStream(ctx context.Context, in *ExecuteToolStreamRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[ExecuteToolStreamResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &PolykeyService_ServiceDesc.Streams[0], PolykeyService_ExecuteToolStream_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[ExecuteToolStreamRequest, ExecuteToolStreamResponse]{ClientStream: stream}
...
type PolykeyService_ExecuteToolStreamClient = grpc.ServerStreamingClient[ExecuteToolStreamResponse]
...
type PolykeyServiceServer interface {
	...
	// ExecuteToolStream runs a tool with streaming responses.
	ExecuteToolStream(*ExecuteToolStreamRequest, grpc.ServerStreamingServer[ExecuteToolStreamResponse]) error
...
func (UnimplementedPolykeyServiceServer) ExecuteToolStream(*ExecuteToolStreamRequest, grpc.ServerStreamingServer[ExecuteToolStreamResponse]) error {
	return status.Errorf(codes.Unimplemented, "method ExecuteToolStream not implemented")
}
...
func _PolykeyService_ExecuteToolStream_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(ExecuteToolStreamRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(PolykeyServiceServer).ExecuteToolStream(m, &grpc.GenericServerStream[ExecuteToolStreamRequest, ExecuteToolStreamResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type PolykeyService_ExecuteToolStreamServer = grpc.ServerStreamingServer[ExecuteToolStreamResponse]
...
var PolykeyService_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "polykey.v1.PolykeyService",
	...
	Streams: []grpc.StreamDesc{
		{
			...
			Handler:       _PolykeyService_ExecuteToolStream_Handler,
			ServerStreams: true,
		},
	},
	Metadata: "polykey/v1/polykey.proto",
}
